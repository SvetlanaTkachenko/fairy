...
(function() {
  var Fairy, Queue, cleanup_required, convertRes, create_client, enter_cleanup_mode, exiting, fairy_id, log_registered_workers, os, prefix, redis, registered_workers, server_ip, uuid, _,
  ...
  Queue = (function() {
    ...
    Queue.prototype.enqueue = function() {
      ...
      return this.redis.multi().rpush(this.key('SOURCE'), JSON.stringify([uuid.v4()].concat(__slice.call(args), [Date.now()]))).sadd(this.key('GROUPS'), args[0]).hincrby(this.key('STATISTICS'), 'TOTAL', 1).exec(function(err, res) {
        res = convertRes(res);
